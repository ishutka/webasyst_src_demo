...
export default {
  ...
  methods: {
    ...
    handleEscapeKeyUp(event) {
      if (event.which === 27 && this.visible) this.$modal.hide(this.name);
    },
    ...
    toggle(nextState, params) {
      ...
      if (beforeEventName === 'before-open') {
        ...
        if (scrollable) document.body.classList.add('v--modal-block-scroll');
      } else if (scrollable)
        document.body.classList.remove('v--modal-block-scroll');
      ...
      if (!stopEventExecution) this.visible = nextState;
    },
    ...
    handleBackgroundClick() {
      if (this.clickToClose) this.toggle(false);
    },
