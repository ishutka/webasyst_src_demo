...
export default lazyManager => {
  return {
    ...
    methods: {
      ...
      checkInView() {
        this.getRect();
        if (inBrowser && window.Cypress === true) return true; // TODO: my pullrequest, to delete when my pulllrequest are merged
        return (
          inBrowser &&
      ...
      load(onFinish = noop) {
        if (this.state.attempt > this.options.attempt - 1 && this.state.error) {
          if (!lazyManager.options.silent)
            console.log(
              `VueLazyload log: ${this.options.src} tried too more than ${
